    ...
    switch (students.get(studentId)) {
      ...
      case (?s) {
        ...
        let updated : StudentTypes.Student = {
          ...
          phone = switch (req.phone) { case (?v) ?v; case null s.phone };
          gender = switch (req.gender) { case (?v) ?v; case null s.gender };
          dob = switch (req.dob) { case (?v) ?v; case null s.dob };
          address = switch (req.address) { case (?v) ?v; case null s.address };
          parent_name = switch (req.parent_name) { case (?v) ?v; case null s.parent_name };
          parent_phone = switch (req.parent_phone) { case (?v) ?v; case null s.parent_phone };
          section = switch (req.section) { case (?v) ?v; case null s.section };
          enrollment_number = switch (req.enrollment_number) { case (?v) ?v; case null s.enrollment_number };
          admission_date = switch (req.admission_date) { case (?v) ?v; case null s.admission_date };
          updated_at = Time.now();
        };
        students.add(studentId, updated);
        ?updated;
